...
if($_SERVER['REQUEST_METHOD'] === "POST"){
    ...
    if($admin) {
        ...
        if(password_verify($_POST['password'], $admin['password_hash'])){
            if($admin['log_status'] == 0 && $admin['email_status'] == 1){
                ...
                if($login_result > 0){
                    ...
                    $_SESSION['admin_log'] = $admin['log_status'];
                    $_SESSION['admin_id'] = $admin['id'];
                    $_SESSION['admin_type'] = $admin['user_type'];
                    $_SESSION['admin_email'] = $admin['email'];
                    $_SESSION['admin_email_status'] = $admin['email_status'];
                    $_SESSION['admin_log_status'] = $admin['log_status'];
            ...
            $admin_id = $admin['id'];
            // echo '<br/>user_id: ' . $admin_id;

            $sql_log = "SELECT * FROM abuloy_user_logins WHERE uid = $admin_id";
    
            $result_log = $mysqli->query($sql_log);
            $admin_log = $result_log->fetch_assoc();
            $row_cnt = mysqli_num_rows($result_log);
            if($row_cnt > 2) {  
                // $sql_log_count = "SELECT * FROM abuloy_user_logins WHERE uid = $admin_id ORDER BY time_count DESC LIMIT 1";
                // $result_log_count = $mysqli->query($sql_log_count);
                // $admin_log_count = $result_log_count->fetch_assoc();
                // $time_secs = 30;
                // $time_count = $admin_log_count['time_count'];
                ...
                $result_update->bind_param('d', $admin_id);
                ...
                $insert_log = "INSERT INTO abuloy_user_logins (uid, ip_add, time_count, date_logged) VALUES ('$admin_id', '$ip_address', '$time', NOW())";
        ...
        if($admin['email_status'] == 0 && $admin['log_status'] == 3){
            $_SESSION['error_login'] = 'Account Locked. Please reset <a href="/forgot-password>here</a>"';
        }
        if($admin['email_status'] == 0 && $admin['log_status'] == 2){
...
//             }
//             elseif($admin['log_status'] === '0'){
//                 session_start();
//                 session_regenerate_id();
//                 $_SESSION['user_log'] = $admin['log_status'];
