        ...
        <div class="row banner-content fullscreen align-items-center justify-content-start pb-4">
            <div class=" col-lg-7 col-md-6 col-sm-12 p-0 mb-4">
                <div class="col-lg-12 pt-5 ps-5">
                    ...
                    <?php 
                        // $userqry = $mysqli->prepare("SELECT * FROM  abuloy_accounts WHERE uid = $uid");
                        // $user_stmt = $userqry->execute();
                        // $user_result = $userqry->get_result();
                        // if($user_row = $user_result->fetch_assoc()){
                        // 
                        $userid = $user['id'];
                        $stmt_account = $mysqli->prepare("SELECT * FROM abuloy_accounts WHERE uid = ?");
                        $stmt_account->bind_param('d', $userid);
                        ...
                        if(isset($account)){
                            if($aid == 1){
                            ?>
                            <div class="pe-5 pt-0"><h4 class="pe-5 pt-0 mt-0 text-blackish">You successfully raised your first fund for <span class="text-lavander"><?= $fname ?> <?= $mname[0] ?>. <?= $lname ?></span></h4></div>
                            <br/>
                            <div class="pe-5 pt-0"><h5 class="pe-5 pt-0 mt-0">Click button below to see your funds now! </h5></div>
                            <a href="/my-funds" class="btn btn-lavander btn-round text-uppercase" id="showFundForm">Go To My Fund <i class="fas fa-chevron-right"></i><i class="fas fa-chevron-right"></i> </a>                             
                            <?php       
                            }
                        }
                        else{ ?>
            ...
            <div class="banner-img-container row align-items-center col-lg-5 col-md-6 col-sm-12 pt-5 pb-100 m-0 px-0 mx-0" id="fundForm">                
                ...
                <a href="/profile_list" class="col-lg-12 img-banner img-fluid m-0 bg-white  px-0 mx-0">
                    <img src="https://abuloy.ph/assets/img/banner-illustration-php.png" alt="" class="img-fluid col-lg-12 my-0  px-0 mx-0" style="height:55vh !important;">
                </a>                
            </div>  
        </div>
    </div>
