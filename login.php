...
if($_SERVER['REQUEST_METHOD'] === "POST"){
    ...
    if($email !== $user['email']){
        $_SESSION['error_login'] = "Email does not exist!";
    }

    $log_time = time() + 3600 * 6;
    // $log_time = date("H:i:s", $time); // 24 hour format
    ...
    if($user) {
        ...
        if(password_verify($_POST['password'], $user['password_hash'])){
            if($user['log_status'] == 0 && $user['email_status'] == 1){
                ...
                $update_log = $mysqli->prepare("UPDATE abuloy_users SET log_status = 1, log_time = '$log_time', token = '$token' WHERE email = '$email'");
                $update_log->execute();
                ...
                if($login_result > 0){
                    ...
                    $_SESSION['fs_provider'] = $user['funeral_provider'];
                    header("Location: /");
                    // exit;
                }
            ...
            elseif($user['log_status'] == 2){
                ...
                $_SESSION['error_login'] = 'You already logged in.<br/>If your did not do this or something goes wrong <br/>please contact customer service or click <a href="/logout/'.$token.'">here</a> to logout properly';
            }
            ...
            $ip_address= "::1"; //store IP Address in variable
            ...
            if($row_cnt > 2) {  
                ...
                $insert_log = "INSERT INTO abuloy_user_logins (uid, ip_add, time_count, token, date_logged) VALUES ('$user_id', '$ip_address', '$time', '$token', NOW())";
...
<body>
    ...
    <section id="login" class="">
        <div class="container mx-auto  login-form-height" >
            <div class="col-lg-12 col-md-12 col-sm-12 text-center" id="login-form">
